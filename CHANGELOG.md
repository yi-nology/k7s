...
- **修复：web 导入的 kubeconfig 重启后丢失。** 浏览器导入（上传/粘贴）的
  配置此前只注册在内存里，容器或进程一重启就全部消失。现在导入成功即把
  原始 YAML 持久化到数据目录的 `web-imports.json`（0600——内容含客户端
  私钥），`k7s-web` 启动时自动重新注册全部导入上下文（日志可见
  `restored N imported kubeconfig context(s)`）；删除导入上下文时同步清理
  其落盘文件，不留私钥垃圾。默认 kubeconfig 的上下文不在此列（来自运维
  挂载的只读文件，始终可用）。
- 版本线：8 子仓同步 v0.7.5 并逐仓打 tag；versions.lock 全量刷新。
