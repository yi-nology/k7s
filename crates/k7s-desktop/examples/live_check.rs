...
//! real cluster, using the same k7s_deps::kube APIs as src/k7s_deps::kube/exec.rs and portforward.rs.
...
async fn main() -> k7s_deps::anyhow::Result<()> {
    ...
    let list = pods
        .list(&ListParams::default().labels("app.k7s_deps::kubernetes.io/name=argocd-redis"))
        .await?;
    let pod = list
        .items
        .into_iter()
