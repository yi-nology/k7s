/**
 * Tests for the palette's result building (B28), written against murphy-yi-shaped
...
const ctx = (over: Partial<PaletteContext> = {}): PaletteContext => ({
  rows: {
    ...
    nodes: [row('murphy-yi'), row('leo'), row('mars')],
...
describe('buildPalette', () => {
  ...
  it('offers cordon only when a node is selected', () => {
    ...
    const with_ = buildPalette('cordon', ctx({ nav: 'nodes', selectedRow: row('murphy-yi') }));
    expect(with_.some((i) => i.type === 'action' && i.id === 'cordon')).toBe(true);
  });

  it('names the node in the action, so what it will do is unambiguous', () => {
    const out = buildPalette('cordon', ctx({ nav: 'nodes', selectedRow: row('murphy-yi') }));
