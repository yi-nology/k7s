...
const base: DrainProgress = { node: "murphy-yi-node-02", evicted: 0, total: 6, failures: [], done: false };
...
describe("MockProvider.drainNode", () => {
  it("reports progress and finishes on a PDB block", async () => {
    ...
    await provider.drainNode("murphy-yi-node-02");
    ...
    expect(last.node).toBe("murphy-yi-node-02");
