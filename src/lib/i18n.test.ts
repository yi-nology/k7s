...
describe('translate', () => {
  ...
  it('resolves table.new and table.newTitle in both locales', () => {
    ...
    expect(translate('en', 'table.newTitle')).toBe('Create a resource from a YAML template');
  ...
  it('resolves the new wizard UI strings in both locales', () => {
    ...
    expect(translate('en', 'tpl.extras.addPlaceholder')).toBe('key=value, then ⏎');
    expect(translate('zh', 'tpl.kind')).toBe('类型');
    expect(translate('zh', 'tpl.section.basic')).toBe('基本');
    expect(translate('zh', 'tpl.cancel')).toBe('取消');
    expect(translate('zh', 'tpl.extras.addPlaceholder')).toBe('key=value, 然后 ⏎');
  });
});
...
describe('helm market repositories panel keys', () => {
  it('ships helm.repos.* and helm.repos.form.* in both locales', () => {
    for (const key of ['refreshAll', 'empty', 'error', 'ok', 'never', 'refresh', 'remove', 'add']) {
  ...
  it('ships helm.empty.{noMatch, noRepos} in both locales', () => {
    expect(translate('en', 'helm.empty.noMatch')).toBe('No charts match this search');
    expect(translate('zh', 'helm.empty.noMatch')).toBe('无匹配的 Charts');
    expect(translate('en', 'helm.empty.noRepos')).toBe('No repos yet — add one in Repositories');
    expect(translate('zh', 'helm.empty.noRepos')).toBe('暂无仓库 — 先在仓库页添加一个');
  });
});
...
describe('action list scale + forward form keys (pass-18)', () => {
  ...
  it('keeps helm.repos.form.nameTitle distinct from the name placeholder (it documents the pattern)', () => {
    ...
    expect(translate('en', 'helm.repos.form.nameTitle')).toBe("lowercase letters, digits, and '-'");
    expect(translate('zh', 'helm.repos.form.nameTitle')).toBe("小写字母、数字与 '-'");
...
describe('settings panel keys', () => {
  ...
  it('ships settings.mcp.sectionTitle, tools, and the three card titles in both locales', () => {
    ...
    for (const key of ['claudeDesktop', 'claudeCode', 'cursor']) {
      expect(
        translate('en', `settings.mcp.${key}.title`).length,
        `settings.mcp.${key}.title en`
      ).toBeGreaterThan(0);
      expect(
        translate('zh', `settings.mcp.${key}.title`).length,
        `settings.mcp.${key}.title zh`
      ).toBeGreaterThan(0);
    }
  });
...
describe('metrics explorer saved-queries strings', () => {
  ...
  it('preserves the canonical refreshTitle values in both locales', () => {
    ...
    expect(translate('en', 'metricsExplorer.refreshTitle')).toBe('Re-run the current query');
    expect(translate('zh', 'metricsExplorer.refreshTitle')).toBe('重新运行当前查询');
  });
  ...
  it("falls back to a custom kind's PascalCase Kind", () => {
    expect(
      kindLabelFor(
        ...
        [{ id: 'argoproj.io/applications', kind: 'Application' }],
        'en'
      )
    ).toBe('Application');
    expect(
...
describe('image registries panel keys', () => {
  ...
  it('ships image.reposEmpty at the documented path (not image.repos.empty)', () => {
    ...
    expect(translate('zh', 'image.reposEmpty')).toBe('无镜像(或仓库不支持 /v2/_catalog)');
...
describe('detail-panel tab + dashboard i18n (pass-15 sweep)', () => {
  ...
  it('ships events.empty in both locales', () => {
    ...
    expect(translate('en', 'events.empty')).toBe('no recent events — events expire after ~1h');
    expect(translate('zh', 'events.empty')).toBe('无最近事件 — 事件约 1 小时后过期');
...
describe('template picker title/description i18n (pass-16 sweep)', () => {
  ...
  it('the fallback path (t() with the registry string as the second arg) renders the registry copy when a key is missing', () => {
    ...
    expect(translate('zh', 'tpl.titles.does-not-exist', fallback)).toBe(fallback);
  });
});
...
describe('chrome statusbar / clusterSwitcher / metricsExplorer.instantTable (pass-17 sweep)', () => {
  ...
  it('ships chrome.clusterSwitcher.{connected,connecting,disconnected,noCluster} in both locales', () => {
    ...
    expect(translate('en', 'chrome.clusterSwitcher.connecting')).toBe('connecting…');
    expect(translate('en', 'chrome.clusterSwitcher.disconnected')).toBe('disconnected');
    expect(translate('en', 'chrome.clusterSwitcher.noCluster')).toBe('no cluster');
    ...
    expect(translate('en', 'chrome.clusterSwitcher.connected', undefined)).toBe('connected');
  });

  it('ships zh cluster-switcher status with 已连接 / 连接中… / 已断开 / 未选择集群', () => {
    expect(translate('zh', 'chrome.clusterSwitcher.connected', 'v1.28.0')).toBe('已连接 · v1.28.0');
    expect(translate('zh', 'chrome.clusterSwitcher.connecting')).toBe('连接中…');
    expect(translate('zh', 'chrome.clusterSwitcher.disconnected')).toBe('已断开');
    expect(translate('zh', 'chrome.clusterSwitcher.noCluster')).toBe('未选择集群');
    // Same undefined-version fallback in zh.
    expect(translate('zh', 'chrome.clusterSwitcher.connected', undefined)).toBe('已连接');

    // Regression: zh does NOT render the English verbs.
    expect(translate('zh', 'chrome.clusterSwitcher.connecting')).not.toContain('connecting');
    expect(translate('zh', 'chrome.clusterSwitcher.disconnected')).not.toContain('disconnected');
  });
  ...
  it('ships metricsExplorer.instantTable.{series,value} in both locales', () => {
    ...
    expect(translate('en', 'metricsExplorer.instantTable.series')).toBe('Series');
    expect(translate('en', 'metricsExplorer.instantTable.value')).toBe('Value');
    expect(translate('zh', 'metricsExplorer.instantTable.series')).toBe('序列');
