...
function parseIngressRows(
  rows: { name: string; namespace?: string; cells: { text: string }[] }[],
) {
  return rows.map((r) => ({
    name: r.name,
...
export function TopologyGraph({
  ...
  const buildGraph = useCallback(async (): Promise<ClusterGraph> => {
    ...
    if (byService.size === 0) {
      ...
      for (const svc of svcRows) {
        ...
        const readyPods = matchingPods.filter(
          (p) => p.pod?.status === "Running",
        );
        nodes.push({
          id: svcId,
          kind: "service",
      ...
      for (const [key, slcs] of byService) {
        ...
        const allAddrs = slcs.flatMap(
          (s) => sliceAddrs.get(`${s.namespace}/${s.name}`) ?? [],
        );
          ...
          for (const addr of allAddrs) {
            ...
            if (!seenPod.has(podKey)) {
              ...
              const podRow = rows.pods.find(
                (r) =>
                  r.namespace === slc.namespace &&
                  r.name === addr.targetRefName,
              );
  ...
  useEffect(() => {
    const sim = forceSimulation<GraphNode>(nodesRef.current)
      .force(
        "charge",
        forceManyBody<GraphNode>().strength(-300).distanceMax(400),
      )
      .force(
        "link",
    ...
    buildGraph().then(({ nodes, links }) => {
      ...
      sim.nodes(nodes);
      (
        sim.force("link") as ReturnType<typeof forceLink<GraphNode, GraphLink>>
      ).links(links);
  ...
  useEffect(() => {
    ...
    if (!node) {
      ...
      node = nodesRef.current.find(
        (n) =>
          n.label === focusedService ||
          `svc:${n.namespace}/${n.label}` === focusedService,
      );
    }
  ...
  useEffect(() => {
    ...
    if (bw < 1 || bh < 1) return;
    const k = clamp(
      Math.min(rect.width / bw, rect.height / bh) * 0.85,
      MIN_ZOOM,
      MAX_ZOOM,
    );
  ...
  const handleMouseDown = useCallback(
    (e: ReactMouseEvent) => {
      ...
      if (target.tagName !== "svg" && !target.closest(`.${styles.canvas}`))
        return;
        ...
        >
          ...
          <g transform={transformStr}>
            ...
            {links.map((l, i) => {
              ...
              const isFlow =
                srcNode.kind === "service" || srcNode.kind === "ingress";
              ...
              const endX =
                (1 - t) * (1 - t) * sx + 2 * (1 - t) * t * cpx + t * t * tx;
              const endY =
                (1 - t) * (1 - t) * sy + 2 * (1 - t) * t * cpy + t * t * ty;
              ...
              return (
                <path
                  ...
                  fill="none"
                  stroke={
                    isHighlight ? "var(--accent, #6366f1)" : "var(--border, #334155)"
                  }
                  strokeWidth={isHighlight ? 2 : 1}
                  opacity={dimmed ? 0.05 : isHighlight ? 0.9 : 0.4}
                  markerEnd={
                    isHighlight ? "url(#arrowhead-hi)" : "url(#arrowhead)"
                  }
            ...
            {nodes.map((n, i) => {
              ...
              const isConnected =
                hover != null && connectedNodeIds.has(n.id);
              const dimmed =
                n._dimmed ||
                (hover != null && !isConnected && hover !== n.id);
        ...
        {tooltip && !dragging && (
          ...
          >
            ...
            <div className={styles.tooltipMeta}>
              {tooltip.node.kind} &middot;{" "}
              {tooltip.node.namespace || "cluster-scoped"}
            </div>
        ...
        <div className={styles.inspector}>
            ...
            <span
              ...
              style={{
                background: selectedNode.unhealthy
                  ...
                  : selectedNode.meta[0] === "Running" ||
                      selectedNode.meta[0] === "Succeeded"
          ...
          <div className={styles.inspectorMeta}>
            ...
            {selectedNode.namespace && (
              <span className={styles.inspectorNs}>
                {selectedNode.namespace}
              </span>
            )}
          </div>
