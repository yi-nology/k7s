...
export function ResourceDiff({ onClose }: { onClose: () => void }) {
  ...
  const fetchRight = useCallback(async () => {
    ...
    setError(null);
    try {
      const yaml = await provider.getYaml(
        toRef(rightKind, rightNs, rightName),
      );
      setRightYaml(yaml);
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : String(e));
    ...
    <div className={styles.diff}>
      ...
      <div className={styles.selectors}>
        ...
        <div className={styles.selectorCol}>
          ...
          <div className={styles.selectorRow}>
            ...
            >
              <option value="">
                {t("diff.selectResource", "Select resource...")}
              </option>
              {leftNameList.map((n) => (
      ...
      {leftYaml && !effectiveRight && (
        <div className={styles.emptyState}>
          {t(
            "diff.emptyHint",
            "Select a resource on the right or paste YAML to see the diff.",
          )}
        </div>
      )}
    </div>
...
function DiffLineRow({ line }: { line: DiffLine }) {
  ...
  return (
    <div className={`${styles.diffLine} ${cls}`}>
      <span className={styles.lineNumLeft}>
        {line.before ?? ""}
      </span>
      <span className={styles.lineNumRight}>
        {line.after ?? ""}
      </span>
