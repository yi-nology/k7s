    ...
    <div style={panelStyle}>
      <header style={headerStyle}>
        <h2 style={{ margin: 0, fontSize: 14 }}>
          {t("ingressEditor.title", "Ingress Editor")}
        </h2>
          ...
          <div style={{ maxWidth: 720 }}>
            ...
            <fieldset style={fieldsetStyle}>
              ...
              <div style={rowStyle}>
                ...
                <label style={labelColStyle}>
                  ...
                  <input value={form.namespace} onChange={(e) => update("namespace", e.target.value)} style={inputStyle} />
                </label>
                <label style={labelColStyle}>
                  {t("ingressEditor.ingressClass", "IngressClass")}
                  <input value={form.ingressClass} onChange={(e) => update("ingressClass", e.target.value)} style={inputStyle} placeholder="nginx" />
                </label>
              </div>
            </fieldset>
            ...
            <fieldset style={fieldsetStyle}>
              <legend style={legendStyle}>
                ...
                <button type="button" style={{ ...btnStyle, marginLeft: 8 }} onClick={() => update("tls", [...form.tls, { secretName: "", hosts: [""] }])}>
              ...
              {form.tls.map((tls, ti) => (
                <div key={ti} style={{ ...rowStyle, alignItems: "flex-end", marginBottom: 6 }}>
                  <label style={labelColStyle}>
                    ...
                    <input value={tls.secretName} onChange={(e) => {
                      const tls2 = [...form.tls]; tls2[ti] = { ...tls2[ti], secretName: e.target.value }; update("tls", tls2);
                    }} style={inputStyle} />
                  </label>
                  <label style={{ ...labelColStyle, flex: 2 }}>
                    {t("ingressEditor.tlsHosts", "Hosts (comma-separated)")}
                    <input value={tls.hosts.join(", ")} onChange={(e) => {
                      const tls2 = [...form.tls]; tls2[ti] = { ...tls2[ti], hosts: e.target.value.split(",").map((s) => s.trim()) }; update("tls", tls2);
                    }} style={inputStyle} />
                  </label>
                  <button type="button" style={{ ...btnStyle, color: "var(--status-err)" }} onClick={() => update("tls", form.tls.filter((_, i) => i !== ti))}>×</button>
                </div>
              ))}
            </fieldset>
            ...
            <fieldset style={fieldsetStyle}>
              <legend style={legendStyle}>
                ...
                <button type="button" style={{ ...btnStyle, marginLeft: 8 }} onClick={() => update("rules", [...form.rules, emptyRule()])}>
                  + {t("ingressEditor.addRule", "Add Rule")}
                </button>
              </legend>
              {form.rules.map((rule, ri) => (
                <div key={ri} style={{ border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-sm)", padding: 8, marginBottom: 8 }}>
                  <div style={rowStyle}>
                    <label style={{ ...labelColStyle, flex: 2 }}>
                      {t("ingressEditor.host", "Host")}
                      <input value={rule.host} onChange={(e) => {
                        const rules2 = [...form.rules]; rules2[ri] = { ...rules2[ri], host: e.target.value }; update("rules", rules2);
                      }} style={inputStyle} placeholder="app.example.com" />
                    </label>
                    <button type="button" style={{ ...btnStyle, color: "var(--status-err)", alignSelf: "flex-end" }} onClick={() => update("rules", form.rules.filter((_, i) => i !== ri))}>×</button>
                  </div>
                  {/* Paths */}
                  {rule.paths.map((path, pi) => (
                    <div key={pi} style={{ ...rowStyle, marginLeft: 16, marginBottom: 4 }}>
                      <label style={labelColStyle}>
                        ...
                        <input value={path.path} onChange={(e) => {
                          const rules2 = [...form.rules]; const paths = [...rules2[ri].paths]; paths[pi] = { ...paths[pi], path: e.target.value }; rules2[ri] = { ...rules2[ri], paths }; update("rules", rules2);
                        }} style={inputStyle} />
                      </label>
                      <label style={labelColStyle}>
                        {t("ingressEditor.pathType", "PathType")}
                        <select value={path.pathType} onChange={(e) => {
                          const rules2 = [...form.rules]; const paths = [...rules2[ri].paths]; paths[pi] = { ...paths[pi], pathType: e.target.value as IngressPath["pathType"] }; rules2[ri] = { ...rules2[ri], paths }; update("rules", rules2);
                        }} style={inputStyle}>
                      ...
                      <label style={labelColStyle}>
                        ...
                        <input value={path.serviceName} onChange={(e) => {
                          const rules2 = [...form.rules]; const paths = [...rules2[ri].paths]; paths[pi] = { ...paths[pi], serviceName: e.target.value }; rules2[ri] = { ...rules2[ri], paths }; update("rules", rules2);
                        }} style={inputStyle} />
                      </label>
                      <label style={{ ...labelColStyle, maxWidth: 80 }}>
                        {t("ingressEditor.port", "Port")}
                        <input type="number" value={path.servicePort} onChange={(e) => {
                          const rules2 = [...form.rules]; const paths = [...rules2[ri].paths]; paths[pi] = { ...paths[pi], servicePort: Number(e.target.value) || 80 }; rules2[ri] = { ...rules2[ri], paths }; update("rules", rules2);
                        }} style={inputStyle} />
                      </label>
                      {rule.paths.length > 1 && (
                        <button type="button" style={{ ...btnStyle, color: "var(--status-err)", alignSelf: "flex-end" }} onClick={() => {
                          const rules2 = [...form.rules]; rules2[ri] = { ...rules2[ri], paths: rule.paths.filter((_, i) => i !== pi) }; update("rules", rules2);
                        }}>×</button>
                      )}
                    </div>
                  ))}
                  <button type="button" style={{ ...btnStyle, marginLeft: 16 }} onClick={() => {
                    const rules2 = [...form.rules]; rules2[ri] = { ...rules2[ri], paths: [...rule.paths, emptyPath()] }; update("rules", rules2);
                  }}>+ {t("ingressEditor.addPath", "Add Path")}</button>
                </div>
              ))}
            </fieldset>
            ...
            <fieldset style={fieldsetStyle}>
              <legend style={legendStyle}>
                ...
                <button type="button" style={{ ...btnStyle, marginLeft: 8 }} onClick={() => update("annotations", [...form.annotations, { key: "", value: "" }])}>
                  + {t("ingressEditor.addAnnotation", "Add")}
                </button>
              </legend>
              {form.annotations.map((ann, ai) => (
                <div key={ai} style={rowStyle}>
                  <input value={ann.key} onChange={(e) => {
                    const a2 = [...form.annotations]; a2[ai] = { ...a2[ai], key: e.target.value }; update("annotations", a2);
                  }} style={{ ...inputStyle, flex: 1 }} placeholder="key" />
                  <input value={ann.value} onChange={(e) => {
                    const a2 = [...form.annotations]; a2[ai] = { ...a2[ai], value: e.target.value }; update("annotations", a2);
                  }} style={{ ...inputStyle, flex: 2 }} placeholder="value" />
                  <button type="button" style={{ ...btnStyle, color: "var(--status-err)" }} onClick={() => update("annotations", form.annotations.filter((_, i) => i !== ai))}>×</button>
                </div>
              ))}
            </fieldset>
          </div>
      ...
      <div style={footerStyle}>
        {dryRunResult && (
          <pre style={{ margin: 0, fontSize: 10, color: dryRunResult.includes("failed") ? "var(--status-err)" : "var(--status-ok)", maxHeight: 60, overflow: "auto" }}>
            {dryRunResult}
          </pre>
        )}
        <div style={{ display: "flex", gap: 6 }}>
          <button type="button" style={btnStyle} disabled={busy} onClick={() => void handleDryRun()}>
            {t("ingressEditor.dryRun", "Dry Run")}
          </button>
          <button type="button" style={{ ...btnStyle, background: "var(--accent)", color: "#fff" }} disabled={busy || !form.name} onClick={() => void handleApply()}>
