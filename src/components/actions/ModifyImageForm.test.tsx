...
const mockGetYaml = vi.fn().mockResolvedValue(
  'apiVersion: apps/v1\nkind: Deployment\nspec:\n  template:\n    spec:\n      containers:\n      - name: app\n        image: nginx:1.25\n'
);
vi.mock('../../providers', () => ({
  getProvider: () => ({
...
describe('ModifyImageForm', () => {
  ...
  it('renders loading state initially', () => {
    view = render(
      <ModifyImageForm ref={resourceRef} onError={vi.fn()} onClose={vi.fn()} />
    );
  ...
  it('calls onClose when cancel is clicked', async () => {
    const onClose = vi.fn();
    view = render(
      <ModifyImageForm ref={resourceRef} onError={vi.fn()} onClose={onClose} />
    );
    await new Promise((r) => setTimeout(r, 100));
