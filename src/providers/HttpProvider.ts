...
function notImplemented(method: string): Promise<never> {
  return Promise.reject(
    new Error(`${method} is not bridged through the browser shell yet`),
  );
}
 ...
 */
export function importKubeconfigViaInput(
  ...
  return new Promise((resolve, reject) => {
    ...
    const onChange = async () => {
      ...
      try {
        ...
        const result = await httpInvoke<ImportResult>(
          "import_kubeconfig_content",
          { filename: file.name, contents },
        );
...
export class HttpProvider implements DataProvider {
  ...
  async startLogs(
    ...
    const offLine = httpSubscribe<{ lines: LogLine[] }>(
      `log-line:${streamId}`,
      (p) => onLines(p.lines),
    );
  ...
  async startShell(
    ...
    const offOut = httpSubscribe<{ data: string }>(
      `shell-out:${streamId}`,
      (p) => onOutput(p.data),
    );
    const offClosed = httpSubscribe<string>(
      `shell-closed:${streamId}`,
      onClosed,
    );
    let stopped = false;
    return {
      input: (data: string) =>
        httpInvoke<void>("shell_input", { streamId, data }),
  ...
  async startNodeShell(
    ...
    const offOut = httpSubscribe<{ data: string }>(
      `shell-out:${info.streamId}`,
      (p) => onOutput(p.data),
    );
    const offClosed = httpSubscribe<string>(
      `shell-closed:${info.streamId}`,
      onClosed,
    );
    let stopped = false;
    return {
      ...
      pod: info.pod,
      input: (data: string) =>
        httpInvoke<void>("shell_input", { streamId: info.streamId, data }),
      resize: (cols: number, rows: number) =>
  ...
  onResourceUpdate(cb: (kind: KindId, rows: Row[]) => void): Unsub {
    const sub = httpSubscribe<{ kind: KindId; rows: Row[] }>(
      "resource-update",
      (payload) => cb(payload.kind, payload.rows),
    );
