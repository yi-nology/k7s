...
use k7s_ios_lib::ai::llm::OpenAiClient;
use k7s_ios_lib::ai::tools::ToolRegistry;
use k7s_ios_lib::ai::LlmClient;
...
async fn make_manager() -> Arc<ClientManager> {
    ...
    let (client, ctx) = k7s_ios_lib::kube::client::build_client("kind-k7s-dev")
        .await
        .expect("build client");
    manager
        .set_connected(
            client,
            k7s_ios_lib::kube::manager::ConnectionInfo {
                context: ctx,
                server: String::new(),
                version: String::new(),
            },
...
async fn run_agent(
    message: &str,
    context: Option<k7s_ios_lib::ai::context::SelectedContext>,
) -> Arc<CollectorSink> {
    ensure_crypto();
    let manager = make_manager().await;
    let llm_factory: Arc<dyn Fn() -> Box<dyn k7s_ios_lib::ai::llm::LlmClient> + Send + Sync> =
...
async fn e2e_deploy_nginx() {
    ...
    let _ = k7s_ios_lib::kube::client::build_client("kind-k7s-dev").await;
...
async fn e2e_diagnose_deployment() {
    let ctx = k7s_ios_lib::ai::context::SelectedContext {
