...
fn make_state() -> k7s_android_lib::web::state::WebState {
    let dir = std::env::temp_dir().join(format!("k7s-test-{}", std::process::id()));
    let _ = std::fs::create_dir_all(&dir);
    k7s_android_lib::web::state::WebState::new(dir, "127.0.0.1:0".parse().unwrap())
}

/// Get the auth token from state.
fn auth_token(state: &k7s_android_lib::web::state::WebState) -> &str {
    &state.web_token
}
...
async fn health_endpoint_returns_ok() {
    ...
    let app = k7s_android_lib::web::server::api_router(state);
