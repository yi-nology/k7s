...
pub fn web_sink_subscribe(sink: &EventSink) -> Option<broadcast::Receiver<WebEvent>> {
    match sink {
        ...
        EventSink::Tauri(_) | EventSink::Mcp(_) => None,
    }
}
...
pub fn web_sink_sender(core: &crate::core::CoreState) -> broadcast::Sender<WebEvent> {
    match core.manager.sink() {
        ...
        EventSink::Tauri(_) => {
            panic!("web_sink_sender: core is wired to TauriEventSink; build a web sink via web_sink() instead")
        }
        EventSink::Mcp(_) => {
            panic!("web_sink_sender: core is wired to McpEventSink; build a web sink via web_sink() instead")
        }
    }
}
